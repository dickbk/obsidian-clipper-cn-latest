...
export async function extractFeishuStructuredContent(doc: Document, url: string = doc.URL): Promise<FeishuStructuredContent | null> {
	if (!isFeishuDocUrl(url)) return null;

	const parsedUrl = parseFeishuUrl(url);
